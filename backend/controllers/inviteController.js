...
import {generateTokenRaw, hashToken} from "../utils/generateInviteToken.js";
...
const invitedProjects = asyncHandler(async (req, res, next) => {
    ...
    const projects = await ProjectMember.findAll({
        ...
        include: [{model: Project, as: 'project', attributes: ['name', 'description']}],
        attributes: {
...
const sendInvitation = asyncHandler(async (req, res, next) => {
    ...
    if (project.ownerId !== inviterId) {
        return next(new ApiError('You cannot invite, this is not your project', 403))
    }

    const invitee = await User.findOne({where: {email}})
    if (!invitee) return next(new ApiError('Invitee not found or registered', 404))
    ...
    if (isAlreadyMember) {
        return next(new ApiError('User to invite is already member of project.', 400))
    }
    ...
    if (existingInvite) {
        return next(new ApiError('An invitation is already pending for this email', 409))
    }

    // generate and hash token and timings
    const rawToken = generateTokenRaw()
    const token = hashToken(rawToken)
    const expiresAt = new Date(Date.now() + 1000 * 60 * 60 * (INVITE_EXPIRATION_HOURS))
    ...
    // const acceptUrl = `${process.env.FRONTEND_URL}/accept-invitation/token=${encodeURIComponent(rawToken)}`;
    //
    // const result = await sendInviteEmail({
    //     to: email,
    //     subject: `Invite to join ${project?.title}`,
...
const verifyInvitation = asyncHandler(async (req, res, next) => {
    ...
    if (invite.acceptedAt) return next(new ApiError('Invite already accepted', 400))
    if (invite.expiresAt < new Date()) return next(new ApiError('Invite expired', 400))
...
const acceptInvitation = asyncHandler(async (req, res, next) => {
    ...
    if (!project) return next(new ApiError('Project to add member to is not found', 401))
